...
def make_train_test_split_of_target(cfg, args, test_size=0.2):
    ...
    if args.target in ['ADNI1', 'ADNI2']:
        ...
        # 【修改点 1】：获取唯一的 Subject 列表，确保每个患者只出现一次
        unique_labels = labels.drop_duplicates(subset=['Subject'])
        
        # 【修改点 2】：对独立的患者名单进行切分（建议加上 random_state=args.seed 保证可重复性）
        x_train, x_test, y_train, y_test = train_test_split(unique_labels['Subject'], 
                                                            unique_labels['Group'], 
                                                            test_size=test_size, 
                                                            stratify=unique_labels['Group'],
                                                            random_state=args.seed
                                                            )
        
        # 【修改点 3】：用划好的患者名单，去原始总表里把他们所有的影像记录提取出来
        train_csv = labels[labels['Subject'].isin(x_train)]
        test_csv = labels[labels['Subject'].isin(x_test)]
        
        test_csv.to_csv(cfg['DATALOADER']['TRAIN_TEST_SPLIT_PATH'] + 'test_' + args.savename + "_" + args.mode + '_seed_' + str(args.seed) + '.csv', index=False)
        train_csv.to_csv(cfg['DATALOADER']['TRAIN_TEST_SPLIT_PATH'] + 'train_' + args.savename + "_" + args.mode + '_seed_' + str(args.seed) + '.csv', index=False)
        ...
        # OASIS3 数据集同理，只不过它的患者列名叫 'OASISID'，标签名叫 'Label'
        unique_labels = labels.drop_duplicates(subset=['OASISID'])
        
        x_train, x_test, y_train, y_test = train_test_split(unique_labels['OASISID'], 
                                                            unique_labels['Label'], 
                                                            test_size=test_size, 
                                                            stratify=unique_labels['Label'],
                                                            random_state=args.seed
                                                            )
        
        train_csv = labels[labels['OASISID'].isin(x_train)]
        test_csv = labels[labels['OASISID'].isin(x_test)]
...
def make_mae_pretraining_dataloaders(cfg, args):
    ...
    if "ADNI1" in args.datasets_to_use:
        datapath_list = datapath_list + glob.glob(cfg['ADNI1']['dataroot'] + 'hdbet_*.nii.gz')
    
    if "ADNI2" in args.datasets_to_use:
        datapath_list = datapath_list + glob.glob(cfg['ADNI2']['dataroot'] + 'hdbet_*.nii.gz')
...
def make_kfold_dataloaders(cfg, args, train_df, test_df):
    ...
    # if need to train with few samples, split in a stratified fashion AT SUBJECT LEVEL
    if cfg["DATALOADER"]["train_size"] in [0.1, 0.2, 0.4, 0.6, 0.8]:

        # 1. 提取训练集中唯一的患者
        unique_train_subjects = train_df.drop_duplicates(subset=['Subject'])
        
        # 2. 按比例抽取患者
        train_subj, _, _, _ = train_test_split(unique_train_subjects['Subject'], 
                                               unique_train_subjects["Group"], 
                                               train_size=cfg["DATALOADER"]["train_size"], # 直接传浮点数比例即可
                                               random_state=args.seed,
                                               shuffle=True, 
                                               stratify=unique_train_subjects["Group"])
        
        # 3. 提取这些患者对应的所有图像
        train_df = train_df[train_df['Subject'].isin(train_subj)]
        
        print(f'Few sample training of {100*cfg["DATALOADER"]["train_size"]} % samples: {len(train_df)} images')
    ...
    for _, row in train_df.iterrows():
        ...
        path_to_file = [x for x in nii_list if str(row['Subject']) in x and str(row['Image Data ID']) in x]
