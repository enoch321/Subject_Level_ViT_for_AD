...
def do_train(cfg, args, FILENAME_POSTFIX, model, criterion, optimizer, scaler, source_loader, 
    ...
    for epoch in range(int(iter_start/iter_per_epoch), epochs):
        ...
        if do_valid:
            test_acc, balanced_acc, test_auc, _, _, _, _, _ = do_inference(cfg, args, model, test_dataloader, logger, is_validation=True)
            model.train()
            if val_acc < test_acc:
                val_acc = test_acc
...
def do_inference(cfg, args, model, test_loader, logger, is_validation=True):
    ...
    # ======== 新增：计算 F1, SEN(Sensitivity/Recall), SPE(Specificity) ========
    # Macro F1 适配多分类和二分类
    test_f1 = f1_score(labels_arr, preds_arr, average='macro')
    
    # 混淆矩阵计算 SEN 和 SPE
    cm = confusion_matrix(labels_arr, preds_arr)
    if cm.shape == (2, 2): # 如果是二分类 (CN vs AD)
        tn, fp, fn, tp = cm.ravel()
        test_sen = tp / (tp + fn) if (tp + fn) > 0 else 0.0
        test_spe = tn / (tn + fp) if (tn + fp) > 0 else 0.0
    else: # 兼容多分类时的 Macro 平均
        sen_list = np.diag(cm) / np.sum(cm, axis=1)
        test_sen = np.nanmean(sen_list)
        spe_list =[]
        for i in range(len(cm)):
            tn = np.sum(cm) - np.sum(cm[i, :]) - np.sum(cm[:, i]) + cm[i, i]
            fp = np.sum(cm[:, i]) - cm[i, i]
            spe_list.append(tn / (tn + fp) if (tn + fp) > 0 else 0.0)
        test_spe = np.nanmean(spe_list)

    logger.info(f'TESTING: Acc {100*test_acc:.2f}%, Bal ACC {100*balanced_acc:.2f}%, AUC {100*test_auc:.2f}%, F1 {100*test_f1:.2f}%, SEN {100*test_sen:.2f}%, SPE {100*test_spe:.2f}%')

    if not is_validation:
        print(f'TESTING: Acc {100*test_acc:.2f}%, Bal ACC {100*balanced_acc:.2f}%, AUC {100*test_auc:.2f}%, F1 {100*test_f1:.2f}%, SEN {100*test_sen:.2f}%, SPE {100*test_spe:.2f}%')

    del logits_arr, labels_arr, preds_arr, auroc_calc
    gc.collect()

    # 原来返回 5 个，现在返回 8 个
    return 100*test_acc, 100*balanced_acc, 100*test_auc, 100*test_f1, 100*test_sen, 100*test_spe, corrects.item(), n_datapoints
