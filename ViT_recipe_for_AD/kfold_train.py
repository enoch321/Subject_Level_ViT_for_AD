...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_file', type=str, default='./configs/config.yaml', help='Name of the config file')
    ...
    for i, (train_index, test_index) in enumerate(skf.split(df, df['Group'])):
        ...
        FILENAME_POSTFIX = f'{args.savename}_{args.source}_mode_{args.mode}_seed_{args.seed}_fold_{i}'
        ...
        test_acc, bal_acc, test_auc, test_f1, test_sen, test_spe, corrects, n_datapoints = do_inference(
        ...
        kfold_results["f1"].append(round(test_f1, 2)) # new
        kfold_results["sen"].append(round(test_sen, 2)) # new
        kfold_results["spe"].append(round(test_spe, 2)) # new
...
print("\n" + "="*40)
print("5-Fold Cross Validation Final Results (Mean ± Std):")
print("="*40)
print(f"Accuracy : {np.mean(kfold_results['acc']):.2f} ± {np.std(kfold_results['acc']):.2f} %")
print(f"Bal Acc  : {np.mean(kfold_results['bal_acc']):.2f} ± {np.std(kfold_results['bal_acc']):.2f} %")
print(f"AUC      : {np.mean(kfold_results['auc']):.2f} ± {np.std(kfold_results['auc']):.2f} %")
print(f"F1-Score : {np.mean(kfold_results['f1']):.2f} ± {np.std(kfold_results['f1']):.2f} %")
print(f"Sensitivity: {np.mean(kfold_results['sen']):.2f} ± {np.std(kfold_results['sen']):.2f} %")
print(f"Specificity: {np.mean(kfold_results['spe']):.2f} ± {np.std(kfold_results['spe']):.2f} %")
print("="*40)
